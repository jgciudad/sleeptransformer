function collection = aggregate_sleeptransformer_local(nchan)
    ...
    aggregate_NREM = 3; % 0 if no aggregation, 1 if aggregating them after the scores aggregation, 2 if aggregating them before the scores aggregation and before the softmax, 3 if aggregating before the scores aggregation but after the softmax
    nchan=1;
    Nfold = 1;
    yh = cell(Nfold,1);
    yt = cell(Nfold,1);
...
%     if(seq_len < 100)
        load("/Users/tlj258/Library/CloudStorage/OneDrive-UniversityofCopenhagen/Documents/PhD/HUMMUSS_paper/outputs/test_tf/eeg1/test_ret.mat");
% 	else
% 	    load(['./intepretable_sleep/sleeptransformer_simple_longseq/scratch_training_',num2str(nchan),'chan/n',num2str(fold),'/test_ret.mat']);
% 	end
                
        if aggregate_NREM == 2 
            score2 = zeros(size(score,1), size(score,2), 3);
            score2(:,:,1) = score(:,:,1);
            score2(:,:,3) = score(:,:,5);
            score2(:,:,2) = sum(score(:,:,2:4),3);
            score = score2;
        end
        ...
        if aggregate_NREM == 3
            for n = 1 : seq_len
                score_before = score{n};
                score_after = zeros(size(score_before, 1), 3);
                score_after(:,1) = score_before(:,1);
                score_after(:,3) = score_before(:,5);
                score_after(:,2) = sum(score_before(:,2:4),2);
                score{n} = score_after;
            end
        end
        ...
        for i = 1 : numel(test_s)
            ...
            for n = 1 : seq_len
                ...
                if aggregate_NREM==2 | aggregate_NREM==3
                    score_i{n} = [ones(seq_len-1,3); score{n}(start_pos:end_pos, :)];
                else
