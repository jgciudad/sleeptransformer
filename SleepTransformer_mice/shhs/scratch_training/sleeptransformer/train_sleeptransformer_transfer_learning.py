# Loads original human model and retrains the classifier (which is the only part that needs to be changed)
...
# My Parameters
tf.app.flags.DEFINE_string("original_human_model", r"C:\Users\javig\Desktop\eval_prueba\test_1chan\n1\checkpoint", "Point to checkpoint directory of original human model")
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        output_layer_variables = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, "output_layer")
        with tf.control_dependencies(update_ops):
            # Define Training procedure
            global_step = tf.Variable(0, name="global_step", trainable=False)
            optimizer = tf.train.AdamOptimizer(config.learning_rate)
            grads_and_vars = optimizer.compute_gradients(net.loss, var_list=output_layer_variables)
        ...
        # variables_dict = {}
        # for v in variables:
        #     variables_dict[v.name[:-2]] = v.get_shape().as_list()

        vars_in_checkpoint = tf.train.list_variables(best_dir)
        print("IN-CHECK-POINT VARIABLES")
        # vars_in_checkpoint_dict = dict(vars_in_checkpoint)
        ...
        var_list_to_retstore = []
        for v in variables:
            if v.name[:-2] in vars_in_checkpoint_names and v.name[:-2].split('/')[0] != 'output_layer': # and v.name[:-2].split('/')[0] != 'global_step':
        #         if variables_dict[v.name[:-2]] == vars_in_checkpoint_dict[v.name[:-2]]:
                var_list_to_retstore.append(v)
        ...
        sess.run(tf.variables_initializer(var_list=tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, "output_layer")))
        sess.run(tf.variables_initializer(var_list=[variables[121]]))
        print("Model initialized")

        saver = tf.train.Saver(tf.all_variables(), max_to_keep=1)

        var_list_to_retstore_before = [v.eval() for v in var_list_to_retstore[:115]]
        output_layer_variables_before = [v.eval() for v in output_layer_variables]
        def compare_weights(list_a, list_b):
            error = []

            for i in range(len(list_a)):
                error.append(np.sum(list_a[i]-list_b[i]))
