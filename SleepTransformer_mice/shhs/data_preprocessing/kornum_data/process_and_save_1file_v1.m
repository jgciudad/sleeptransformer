...
%     mat_path = './mat/';
    ...
    stages = tsvread([scorer_folder, '\tsv\', name, '.tsv']);
    cutrows = find(isnan(stages(:,1))==0, 1, 'first');
    stages = stages(cutrows:end,5);
    stages((stages~=1) & (stages~=2) & (stages~=3)) = 4; % ALL ARTIFACTS TO LABEL 4
    [eeg1, ori_fs] = read_shhs_edfrecords([scorer_folder, '\edf\', name, '.EDF'], {'EEG'}, {1});
    ...
    [eeg2, ori_fs] = read_shhs_edfrecords([scorer_folder, '\edf\', name, '.EDF'], {'EEG'}, {2});
    if(ori_fs ~= fs) % resampling
    ...
    [emg, ori_fs] = read_shhs_edfrecords([scorer_folder, '\edf\', name, '.EDF'], {'EMG'}, {3});
    if(ori_fs ~= fs) % resampling
