...
%     end
    ...
    [eeg, ori_fs] = read_shhs_edfrecords([scorer_folder, '\edf\', name, '.EDF'], {'EEG'}, {1});
...
%     eog = filtfilt(b_band,1,eog);
    ...
    [emg, ori_fs] = read_shhs_edfrecords([edf_path, filepath], {'EMG'});
    if(ori_fs ~= fs) % resampling
...
%     inf_ind = (isinf(sum(X_eeg_')) | isinf(sum(X_eog_')) | isinf(sum(X_emg_')));
    inf_ind = (isinf(sum(X_eeg_')) | isinf(sum(X_emg_')));
    count = sum(inf_ind);
